// npm install gulp gulp-watch typescript gulp-typescript gulp-livereload gulp-concat gulp-cssmin gulp-jade gulp-stylus gulp-rename gulp-flatten gulp.spritesmith gulp-uncss gulp-jsmin
...
gulp.task('bb8-build', function () {
    gulp.run(['bb8-vendor-js', 'bb8-js', 'bb8-css']);
});

gulp.task('bb8-css', function(){
    gulp.src([
            'src/stylus/viewer.styl'
        ])
        .on('error', console.log)
        .pipe(stylus({compress: true}))
        .pipe(rename({suffix: '.min'}))
        .pipe(gulp.dest('D:/OpenServer/domains/sites/bb8/backend/application/views/vendor/css'))
});
...
gulp.task('bb8-js', function(){
    gulp.src([
        ...
        .pipe(gulp.dest('D:/OpenServer/domains/sites/bb8/backend/application/views/vendor/js'))
});

gulp.task('bb8-vendor-js', function(){
    gulp.src([
            'src/js/vendor/*.js'
        ])
        .on('error', console.log)
        .pipe(gulp.dest('D:/OpenServer/domains/sites/bb8/frontend/src/js/vendor/'))
});
